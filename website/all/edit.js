...
function updateMachine() {
  ...
  fetch(
    "https://testapi.robli.at/machine/update/" + localStorage.getItem("id"),
    {
      method: "PUT",
      body: JSON.stringify({
        name: name,
...
async function showMachineNames() {
  ...
  document.getElementById("name").placeholder =
    data[localStorage.getItem("update")].name;
  document.getElementById("surname").placeholder =
    data[localStorage.getItem("update")].surname;
  document.getElementById("email").placeholder =
    data[localStorage.getItem("update")].email;
  ...
  for (let i = 0; i < data[localStorage.getItem("update")].allow.length; i++) {
    document.getElementById(
      data[localStorage.getItem("update")].allow[i]
    ).checked = true;
  }
}
